...
void define_structure(void)
{
	...
	if (sym != ident)
		...
		if (exist(id, structdef))
			...
			do {
				if ((sym != bytesym) && (sym != shortintsym)
						&& (sym != addresssym) && (sym != longintsym)
					...
					if (mem_type == undefined)
						...
						if (sym != ident) {
							...
							add_struct_member(structdef_item, id, mem_type,
									struct_mbr_def);
							mem_count++;
						}
					}
					...
					if ((mem_type == stringtype) && (sym == sizesym)) {
						...
						if (sym == shortconst)
							...
							if (curr_item->type == shorttype)
								string_size =
										(LONG) curr_item->numconst.shortnum;
			...
			if (mem_count == 0)
				structdef_item->object = undefined;
		}
	}
		...
		if (sym != ident)
			...
			if (sym != equal)
				...
				if (exist(const_id, constant)) /* don't try to reassign it! */
					...
					if ((sym == ident) && (exist(id, constant))) {
						...
						if (consttyp != stringtype) {
							...
							switch (consttyp) {
							case shorttype:
								shortc = curr_item->numconst.shortnum;
								break;
							case longtype:
								longc = curr_item->numconst.longnum;
								break;
							case singletype:
								singlec = curr_item->numconst.singlenum;
								break;
							}
						} else
							_error(27);
				...
				if (numconstant) {
					switch (consttyp) {
					case shorttype:
						enter(const_id, shorttype, constant, 0);
						if (negate)
							shortc *= -1;
					...
					case longtype:
						enter(const_id, longtype, constant, 0);
						if (negate)
							longc *= -1;
					...
					case singletype:
						enter(const_id, singletype, constant, 0);
						if (negate)
							singlec = SPMul(singlec, smul);
