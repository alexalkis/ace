...
void get_event_trap_label(void)
{
	...
	if ((sym != breaksym) && (sym != menusym) && (sym != mousesym)
		...
		if (event == timersym) {
			...
			if (sym != lparen)
				...
				if ((sym != shortconst) && (sym != longconst)
					...
					switch (sym) {
					case shortconst:
						timer_event_seconds = SPFlt((ULONG ) shortval);
						break;
					case longconst:
						timer_event_seconds = SPFlt((ULONG ) longval);
						break;
					case singleconst:
						timer_event_seconds = singleval;
						break;
					}
...
void change_event_trapping_status(int event)
{
	...
	if (lastsym != windowsym && lastsym != gadgetsym && lastsym != menusym)
		insymbol();
	...
	if ((sym != onsym) && (sym != offsym) && (sym != stopsym))
		...
		if (action == offsym)
			...
			case breaksym:
				if (break_event_label_exists) {
					break_event = FALSE;
					break_event_label_exists = FALSE;
				} else
					_error(59);
				break;
			...
			case mousesym:
				if (mouse_event_label_exists) {
					mouse_event = FALSE;
					mouse_event_label_exists = FALSE;
				} else
					_error(59);
				break;
			...
			case timersym:
				if (timer_event_label_exists) {
					timer_event = FALSE;
					timer_event_label_exists = FALSE;
				} else
					_error(59);
				break;
			...
			case errorsym:
				if (error_event_label_exists) {
					error_event = FALSE;
					error_event_label_exists = FALSE;
				} else
					_error(59);
				break;
...
void turn_event_off(char *eventHandler)
{
	...
	if ((strcmp(eventHandler, break_event_label) == 0) && (break_event)) {
		...
		menu_event = FALSE;
		menu_event_label_exists = FALSE;
	} else if ((strcmp(eventHandler, mouse_event_label) == 0)
			&& (mouse_event)) {
		mouse_event = FALSE;
		mouse_event_label_exists = FALSE;
	} else if ((strcmp(eventHandler, timer_event_label) == 0)
			&& (timer_event)) {
		timer_event = FALSE;
		timer_event_label_exists = FALSE;
	} else if ((strcmp(eventHandler, error_event_label) == 0)
			&& (error_event)) {
		error_event = FALSE;
		error_event_label_exists = FALSE;
...
void break_event_test(void)
{
	...
	else if (break_event_branch == gosubsym)
...
void menu_event_test(void)
{
	...
	else if (menu_event_branch == gosubsym)
...
void mouse_event_test(void)
{
	...
	else if (mouse_event_branch == gosubsym)
...
void timer_event_test(void)
{
	...
	else if (timer_event_branch == gosubsym)
...
void error_event_test(void)
{
	...
	else if (error_event_branch == gosubsym)
...
void wdw_event_test(void)
{
	...
	else if (wdw_event_branch == gosubsym)
...
void gad_event_test(void)
{
	...
	else if (gad_event_branch == gosubsym)
