...
{
	if (opcode[0] == 'm') {
		if (strcmp(opcode, "move.b") == 0)
			return (TRUE);
		else if (strcmp(opcode, "move.w") == 0)
			return (TRUE);
		else if (strcmp(opcode, "move.l") == 0)
			return (TRUE);
		else
			return (FALSE);
	} else
...
BOOL push_pop_pair(void)
{
	...
	if (strcmp(first->opcode, second->opcode) == 0 && is_a_move(first->opcode)
...
BOOL sign_bit_extension(void)
{
	...
	if (strcmp(second->opcode, "ext.l") == 0
		...
		peep++;
		return (TRUE);
	} else if (strcmp(second->opcode, "ext.w") == 0
...
void optimise(void)
{
	...
	printf("\noptimising...\n");
	opeep = peephole();
	printf("%d peephole ", opeep);
	if (opeep == 1)
		printf("removal.");
	else
		printf("removals."); /* opeep==0 or opeep>1 */
}
