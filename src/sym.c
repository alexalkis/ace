...
BOOL exist(char *name, int obj)
{
	...
	if ((obj == subprogram) || (obj == function) || (obj == definedfunc)
			|| (obj == extfunc) || (obj == extvar) || (obj == constant)
			|| (obj == structdef))
		lev = ZERO;
...
/* enter a symbol into symbol table */
{
	...
	if ((new_item->name = (char *) sym_alloc(strlen(name) + 1, MEMF_ANY))
			== NULL) {
		printf("Can't allocate memory for symbol table item!\n");
		early_exit = TRUE;
		kill_all_lists();
	...
	if (obj == array) {
		if ((new_item->index = (SHORT *) sym_alloc((dims + 1) * 2, MEMF_ANY))
				== NULL) {
	...
	if (obj == structdef) {
		if ((new_item->structmem = (STRUCM *) sym_alloc(sizeof(STRUCM),
				MEMF_ANY)) == NULL) {
...
void enter_DATA(char *name, char *literal)
{
	...
	if ((new_data->literal = (char *) alloc(strlen(literal) + 1, MEMF_ANY))
			== NULL) {
...
void enter_BASDATA(char *literal)
{
	...
	if ((new_basdata->literal = (char *) alloc(strlen(literal) + 1, MEMF_ANY))
			== NULL) {
