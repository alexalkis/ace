...
BOOL factorfunc(void)
{
	...
	case possym:
		return (TRUE);
	case rndsym:
		return (TRUE); /* has optional parameter! */
	case systemsym:
		return (TRUE);
	case timersym:
		return (TRUE);
...
int factor(void)
{
	...
	case ident: /* does object exist? */
		...
		if (exist(id, array)) {
			obj = array;
			arraytype = typ = curr_item->type;
		} else if (exist(sub_name, subprogram)) {
			obj = subprogram;
			typ = curr_item->type;
		} else if (exist(sub_name, definedfunc)) {
			obj = definedfunc;
			typ = curr_item->type;
		} else if (exist(func_name, function)) {
			obj = function;
			typ = curr_item->type;
		} else if (exist(ext_name, extfunc)) {
			obj = extfunc;
			typ = curr_item->type;
			...
			obj = structure;
		else if (exist(id, constant)) {
			obj = constant;
			typ = curr_item->type;
		} else if (exist(id, obj)) /* obj == variable? */
			typ = curr_item->type;
		else {
