...
BOOL strfunc(void)
{
	...
	case hexsym:
		return (TRUE);
	case inputboxsym:
		return (TRUE); /* this is here for convienience */
	case inputboxstrsym:
		return (TRUE);
	case inputstrsym:
		return (TRUE);
	...
	if (strfunc()) {
		...
		if (sym != lparen)
			...
			case binstrsym:
				...
				/* CSTR */
			case cstrsym:
				if ((sftype == stringtype) || (sftype == longtype))
					sftype = stringtype;
				else {
					_error(4);
					sftype = undefined;
...
int numericfunction(void)
{
	...
	if (numfunc()) {
		...
		if (sym != lparen)
			...
			insymbol();
			if ((func != varptrsym) && (func != sizeofsym))
				nftype = expr();
			...
			case allocsym:
				...
				/* ATN */
			case atnsym:
				nftype = gen_single_func("_LVOSPAtan", nftype);
				break;
			...
			case clngsym:
				if (nftype == singletype) {
					gen_round(nftype);
					nftype = longtype;
				} else if (nftype == shorttype) {
					gen("move.w", "(sp)+", "d0");
					gen("ext.l", "d0", "  ");
					gen("move.l", "d0", "-(sp)");
					nftype = longtype;
				} else if (nftype == stringtype) {
					_error(4);
					nftype = undefined;
				}
				break;
				...
				/* COS */
			case cossym:
				nftype = gen_single_func("_LVOSPCos", nftype);
				break;

				/* CSNG */
			case csngsym:
				if ((nftype == shorttype) || (nftype == longtype)) {
					gen_Flt(nftype);
					nftype = singletype;
				} else if (nftype == stringtype) {
					_error(4);
			...
			case eofsym:
				...
				/* EXP */
			case expsym:
				nftype = gen_single_func("_LVOSPExp", nftype);
				break;
			...
			case lofsym:
				...
				/* LOG */
			case logsym:
				nftype = gen_single_func("_LVOSPLog", nftype);
				break;
			...
			case shrsym:
				...
				/* SQR */
			case sqrsym:
				nftype = gen_single_func("_LVOSPSqrt", nftype);
				break;

				/* SIN */
			case sinsym:
				nftype = gen_single_func("_LVOSPSin", nftype);
				break;

				/* SIZEOF */
			case sizeofsym:
				nftype = find_object_size();
				break;
			...
			case strigsym:
				...
				/* TAN */
			case tansym:
				nftype = gen_single_func("_LVOSPTan", nftype);
				break;
...
int address_of_object(void)
{
	...
	if (exist(extobjid, extvar) || exist(extobjid, extfunc)) {
		gen("pea", extobjid, "  ");
		return (longtype);
	...
	if (sym == ident) {
		...
		if (exist(id, variable)) {
			if (curr_item->type == shorttype) {
				...
				gen("move.l", "#4", "-(sp)");
				nftype = longtype;
			} else if (curr_item->type == singletype) {
				gen("move.l", "#4", "-(sp)");
				nftype = longtype;
			} else if (curr_item->type == stringtype) {
