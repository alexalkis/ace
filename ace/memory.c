...
void get_obj_info(char *objname, int *object, int *objtype)
{
	...
	if (exist(objname, variable))
		found = TRUE;
	else if (exist(objname, extvar))
		found = TRUE;
	else if (exist(objname, array))
		found = TRUE;
	else if (exist(objname, structure))
		found = TRUE;
...
void swap(void)
{
	...
	if (sym != ident)
		...
		if (dataobj1 == structure)
			typ1 = struct_member_type;

		if (dataobj1 != structure && dataobj1 != array)
			insymbol();
		...
		if (sym != comma)
			...
			if (sym != ident)
				...
				/* get_obj_info() won't tell us about structure member type */
				if (dataobj2 == structure)
					typ2 = struct_member_type;

				if (dataobj2 != structure && dataobj2 != array)
					insymbol();
