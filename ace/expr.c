...
extern long labelcount;
extern char tempstrname[80];

/* functions */
BOOL coerce(int *typ1, int *typ2, CODE * cx[])
{
	if ((*typ1 == stringtype) && (*typ2 != stringtype))
		return (FALSE);
	else if ((*typ2 == stringtype) && (*typ1 != stringtype))
		return (FALSE);
	else if (((*typ1 == shorttype) || (*typ1 == longtype))
			&& (*typ2 == singletype)) {
		change_Flt(*typ1, cx);
		*typ1 = singletype;
		return (TRUE);
	} else if (((*typ2 == shorttype) || (*typ2 == longtype))
			&& (*typ1 == singletype)) {
		gen_Flt(*typ2);
		*typ2 = singletype;
		return (TRUE);
	} else if ((*typ1 == shorttype) && (*typ2 == longtype)) {
		change(cx[0], "move.w", "(sp)+", "d0");
		change(cx[1], "ext.l", "d0", "  ");
		change(cx[2], "move.l", "d0", "-(sp)");
		*typ1 = longtype;
		return (TRUE);
	} else if ((*typ2 == shorttype) && (*typ1 == longtype)) {
		gen("move.w", "(sp)+", "d0");
		gen("ext.l", "d0", "  ");
		gen("move.l", "d0", "-(sp)");
		*typ2 = longtype;
...
int simple_expr(void)
{
	...
	while ((sym == plus) || (sym == minus)) {
		...
		if (coercion) {
			switch (op) {
			case plus:
				if ((modtype != stringtype) && (modtype != shorttype)) {
					gen("move.l", "(sp)+", "d1");
					gen("move.l", "(sp)+", "d0");
				}
...
BOOL relop(int op)
{
	if ((op == equal) || (op == notequal) || (op == gtrthan) || (op == lessthan)
			|| (op == gtorequal) || (op == ltorequal))
		return (TRUE);
	else
		return (FALSE);
}
